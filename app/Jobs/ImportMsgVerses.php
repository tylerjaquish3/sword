...
class ImportMsgVerses implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach ($books as $book) {
            foreach ($book->chapters as $chapter) {
                ...
                if ($processed >= $this->limit) {
                    Log::info("ImportMsgVerses: reached limit of {$this->limit}, stopping. Run again to continue.");
                    return;
                }
                ...
                try {
                    ...
                    if (empty($verses)) {
                        Log::warning('ImportMsgVerses: no verses parsed for ' . $query);
                        $processed++;
                        continue;
                    }
                    ...
                    Log::info("ImportMsgVerses: {$query} — {$versesCreated} verses created ({$processed}/{$this->limit} this run)");
                    $processed++;
                    ...
                    Log::error('ImportMsgVerses: failed on ' . $query, [
        ...
        Log::info('ImportMsgVerses: all chapters imported.');
    }
    ...
    private function parseVerses(string $html): array
    {
        ...
        // We intentionally do NOT filter by the en-MSG- id: verse 1 and poetry continuation
